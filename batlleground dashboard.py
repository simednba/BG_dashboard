...
def render_general_page1(t_1, n_max, n_min):
    if t_1 =='MMR':
        ...
        return html.Div(render_graph(x,y,titre = 'Placements', fig_title = f'Winrate global : {df_top.loc["global"]["winrate"]}<br>, Placement moyen :{sum([x/100*(i+1) for i,x in enumerate(df_top.loc["global"].values[1:-1])])} '))
    else:
